...
router.post("/register", async (req, res) => {
  try {
    const { email, password, username, full_name, gender, country_code, date_of_birth } = req.body;
    ...
    const seedLocation = centroid ? fuzzLocation(centroid.latitude, centroid.longitude) : null;
    const createData = {
      email,
      ...
      ...(seedLocation ? { latitude: seedLocation.latitude, longitude: seedLocation.longitude } : {}),
    };
    if (gender) createData.gender = gender;
    if (date_of_birth) {
      const dobDate = new Date(date_of_birth);
      if (!isNaN(dobDate.getTime())) createData.date_of_birth = dobDate;
    }
...
router.post("/google", async (req, res) => {
  try {
    ...
    if (firebaseAdmin) {
      ...
      console.warn("[auth/google] Firebase Admin not initialized — dev mode");
    ...
    if (user && googlePicture && !user.profile_picture_url) {
      try {
        user = await prisma.user.update({
          where: { id: user.id },
          data: { profile_picture_url: googlePicture },
        });
      } catch (_) {}
    }
...
// POST /auth/refresh  (rotation: old refresh token is revoked on every use)
router.post("/refresh", async (req, res) => {
  try {
    ...
    const { access_token, refresh_token: newRefresh, refresh_expires_at } =
...
router.post("/logout", async (req, res) => {
  try {
    ...
    res.json({ message: "Logged out", ok: true });
  } catch (error) {
...
router.post("/logout-all", authenticateToken, async (req, res) => {
  try {
    await revokeRefreshToken(null, { all: true, userId: req.user.id });
    res.json({ message: "All sessions revoked", ok: true });
  } catch (error) {
...
router.post("/send-verification", authenticateToken, async (req, res) => {
  try {
    ...
    res.json({ message: "Verification email sent", ok: true });
  } catch (error) {
...
router.post("/verify-email", authenticateToken, async (req, res) => {
  try {
    ...
    if (user.is_email_verified) {
      return res.json({ message: "Email already verified", ok: true });
    }
    if (
