...
function parseGoogleApiError(error) {
  // GaxiosError shape: { code, status, response: { status, data: { error: { code, status, message, errors: [{ reason }] } } } }
  const httpStatus = error?.response?.status || error?.code;
  const apiError = error?.response?.data?.error;
  const reason = apiError?.errors?.[0]?.reason || apiError?.status;

  if (httpStatus === 404 || reason === "purchaseTokenNotFound" || reason === "purchaseTokenInvalid") {
    return BillingError(
...
async function verifyPurchase(productId, purchaseToken) {
  ...
  if (item.kind === "subscription") {
    ...
    if (data.paymentState === 0) {
      throw BillingError("PURCHASE_PENDING", "Subscription payment is pending", 400);
    }
    // No expiry or already expired → not a usable subscription.
    if (!data.expiryTimeMillis || parseInt(data.expiryTimeMillis, 10) <= Date.now()) {
      throw BillingError("SUBSCRIPTION_EXPIRED", "Subscription is not active or is expired", 400);
    }
  ...
  // purchaseState 0 = purchased (payment completed). 1 = canceled, 2 = pending.
  if (data.purchaseState !== 0) {
    throw BillingError("PURCHASE_NOT_COMPLETE", "Purchase is not completed by Google", 400);
  }
...
async function verifyAndGrantPurchase(
  ...
  verify = verifyPurchase // injectable for tests
) {
  if (!productId || !purchaseToken) {
    throw BillingError("MISSING_FIELDS", "productId and purchaseToken are required", 400);
  }
  ...
  if (existing) {
    throw BillingError("PURCHASE_TOKEN_USED", "This purchase token has already been used", 400);
  }
  ...
  const amount =
    kind === "product"
      ? Object.values(item.grant).reduce((sum, value) => sum + (value || 0), 0)
      : 0;
  ...
  try {
    ...
    // P2002 = unique constraint violation on purchaseToken → concurrent replay.
    if (error?.code === "P2002" || error?.meta?.target?.includes?.("purchaseToken")) {
      throw BillingError("PURCHASE_TOKEN_USED", "This purchase token has already been used", 400);
    }
    throw error;
  }
